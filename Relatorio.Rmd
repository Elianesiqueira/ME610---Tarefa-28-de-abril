...
- Idade_pai: Idade do pai (anos);

Afim de avaliar a influência das demais variáveis sobre o peso do bebê, foi considerado o peso da criança ao nascer como variável resposta e as demais variáveis como explicativas.  
...
As variáveis referentes a raça da mãe e do pai passaram a ter seis níveis, onde white(1),white(2),white(3),white(4),white(5) passaram a pertencer a um único nível white.   
...
Estado civil passou a ser classificado apenas como "casada" e "não casada" e a variável correspondente ao hábito de fumar passou a ter três níveis, pois as mães que haviam fumado até a gravidez atual e as que fumavam passaram a ser englobadas em um só nível, "Fumava".  
...
Excluindo-se as variáveis indicadas como não significativas pelo ajuste anterior obteve-se o segundo modelo ao qual indicou que os níveis "Fumava" (referente a variável "Fuma"), "Não definido" (referente a variável "Cor Pai") e "Mex","Mixed" e "Não Definido" (referentes a variável "Cor Mãe") não eram estatisticamente significantes ao nível de 10% de significância, ou seja, estes níveis não eram estatisticamente diferentes dos seus respectivos níveis de referência "Nunca" (refente a variável "Fuma"), "Asian" (referente a variável "Cor do Pai") e "Asian" (referente a variável "Cor da mãe"). 
Dados os resultados do segundo modelo, a fim de se ajustar um terceiro modelo mais adequado que os anteriores,  agrupou-se os níveis "Fumava" e "Nunca" da variável "Fuma" (formando um novo nível também denominado "Nunca/Fumava"), assim como para os níveis "Asian" e "Não definido" da variável "Cor do Pai" (formando um novo nível "Asian/Não definido") e para os níveis "Asian", "Mex","Mixed" e "Não Definido" da variável "Cor da Mãe" (formando um novo nível "Asian/Mex/Mixed/Não Definido").  
O terceiro modelo foi então ajustado com as mesmas variáveis que o segundo, porém com as alterações nos níveis indicadas. Deste modelo observou-se que os níveis "Black" (referente a variável "Cor Pai") e "white" (referente a variável "Cor Mãe") não eram estatisticamente diferentes dos respectivos níveis de referência "Asian/Não definido" (referente a variável "Cor do Pai") e "Asian/Mex/Mixed/Não Definido" (referente a variável "Cor da Mãe"), portanto, como anteriormente, agrupou-se os grupos em "Asian/Não definido/Black" (referentes a cor do pai) e em "Asian/Mex/Mixed/Não Definido/white" (referentes a cor da mãe).  
Com as devidas alterações nos níveis das variáveis citadas, e com as mesmas variáveis do terceiro modelo, foi-se ajustado o quarto modelo, onde observou-se que o nível "Black" referente a variável "Cor da Mãe" não era estatisticamente diferente do grupo de referência que engloba todos os outros níveis da variável "Cor da Mãe", portanto, a variável "Cor da Mãe", considerando o critério adotado, não era estatisticamente significante, e portanto, deve ser retirada de um próximo modelo.  

Por fim, um quinto modelo foi ajustado com as mesmas variáveis do quarto modelo com exceção da variável "Cor da mãe". Este modelo nos indicou, segundo o critério adotado, que é um modelo não redutível, portanto é o modelo mais simples que podemos obter seguindo os critérios mencionados, e por isso, este será usado como modelo de referência para a análise do poder explicativo das variáveis explanatórias em relação a variável resposta.  

Modelo de referência:  

  $$Peso_i = \beta_0 + \beta_1Fuma_i + \beta_2Mex_i + \beta_3Mixed_i + \beta_4White_i + \beta_5 \left ( \frac{P_i-\bar{P}}{\sigma_P}   \right )+ \beta_6 \left ( \frac{G_i-\bar{G}}{\sigma_G}   \right )+ \beta_7 \left ( \frac{N_i-\bar{N}}{\sigma_N}   \right )+ \beta_8 \left ( \frac{NG_i-\bar{NG}}{\sigma_{NG}}   \right ) + \beta_9 \left ( \frac{A_i-\bar{A}}{\sigma_{A}}   \right ) + \varepsilon_i $$  
Onde:  
...
Medidas de poder preditivo do modelo:  
$R^2$ = 0.2997  
$\bar{R}^2$ = 0.2936  
