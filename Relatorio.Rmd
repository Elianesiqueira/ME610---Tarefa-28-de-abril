...
Após a adequação dos dados, foi feita uma análise descritiva, onde foi feito boxplot para variáveis categóricas e gráfico de disperção para as variáveis contínuas e discretas. Analisando os boxplots notou-se para algumas variáveis, que alguns níveis tem distribuição muito semelhante, sugerindo a igualdade entre eles, a partir disso, os níveis que apresentaram tais características e faziam sentido em termos do problema, foram reagrupados em um mesmo nível e seus gráficos foram refeitos com as devidas alterações. As variáveis relativas ao hábito de fumar e ao tempo sem fumar, após as adequações, contiveram as mesmas informações, por isso foi mantida apenas a variável relativa ao hábito de fumar.

### 4. Metodologia



Buscando atender o objetivo do estudo,  e a fim de que as inferências sejam válidas mesmo com presença de heterocedásticidade nos dados, propõe-se um modelo de regressão linear multipla com erros padrão robustos (White standard errors) . Um primeiro modelo foi ajustado com todas as variáveis não excluidas após a preparação dos dados, e a partir da observação das estimativas dos coeficientes do modelo e seus respectivos testes de nulidade, reajustou-se um novo modelo readaptando os dados de maneira a representar melhor os niveis de cada variável e excluindo-se variáveis não significantes, ou seja, reagrupou-se os niveis das variáveis não significativas (a um nível de significancia de 10% ) ao nível de refencia do modelo, e não contemplou-se neste novo modelo as variáveis onde todos os seus níveis tiveram coefientes estatísticamente não significativos (a um nível de significancia de 10%). Procedeu-se assim até obter-se um modelo irredutível em termos do procedimento descrito. Por fim, interpretou-se os resultados obtidos pelo modelo irredutível de forma a obter uma resposta ao objetivo.
...
O nivel educacional do pai e da mãe passaram a ter cinco níveis, HS+trade e Trade school HS unclear foram incorporadas em Trade, enquanto que less than 8th grade e 8th-12th grade foram incorporadas em less than 12th grade.
...
Após os dados sofrerem as alterações advindas das análises descritivas, inicia-se a análise inferêncial. 
O primeiro modelo ajustado, que contempla todas as variáveis indicadas pela análise descritiva indicou, segundo o critério adotado, que as variáveis "Tempo sem Fumar" , "Rendimento Anual", "Estado Civil", "Educação Pai", "Educação Mãe","idade da mãe" e "idade do pai" não são estatísticamente significativas, e portanto não devem ser inseridas no novo modelo a ser ajustado.
Excluindo-se as variáveis indicadas como não significativas pelo ajuste anterior obteve-se o segundo modelo ajustado ao qual indicou que os níveis "Fumava" (referente a variável "Fuma"), "Não definido" (referente a variável "Cor Pai") e "Mex","Mixed" e "Não Definido" (referentes a variável "Cor Mãe") não são estatísticamente significantes ao nível de 10% de significancia, ou seja, estes níveis não são estatísticamente diferentes dos seus respectivos níveis de referência "Nunca" (refente a variável "Fuma"), "Asian" (referente a variável "Cor do Pai") e "Asian" (referente a variável "Cor da mãe"). 
...
O terceiro modelo foi então ajustado com as mesmas variáveis que o segundo, porém com as alterações nos níveis indicadas. Deste modelo observou-se que os nível "Black" (referente a variável "Cor Pai") e "white" (referente a variável "Cor Mãe") não são estatísticamente diferentes dos respectivos níveis de referência "Asian/Não definido" (referente a variável "Cor do Pai") e "Asian/Mex/Mixed/Não Definido" (referente a variável "Cor da Mãe"), portanto, como anteriormente, agrupou-se os grupos em "Asian/Não definido/Black" (referentes a cor do pai) e em "Asian/Mex/Mixed/Não Definido/white" (referentes a cor da mãe).
Com as devidas alterações nos níveis das variáveis citadas, e com as mesmas variáveis do terceiro modelo, foi-se ajustado o quarto modelo, onde observou-se que o nível "Black" referente a variável "Cor da Mãe" não é estatísticamente diferente do grupo de referência que engloba todos os outros níveis da variável "Cor da Mãe", portanto, a variável "Cor da Mãe", considerando o critério adotado, não é estatísticamente significante e deve ser retirada de um próximo modelo.

Por fim, um quinto modelo foi ajustado com as mesmas variáveis do quarto modelo com excessão da variável "Cor da mãe". Este modelo nos indicou, segundo o critério adotado, que é um modelo não redutível, portanto é o modelo mais simples que podemos obter seguindo os critérios mencionados, e por isso, este será usado como modelo de referência para a análise do poder explicativo das variáveis explanatórias em relação a variável resposta.
...
$\beta_0$: representa o parâmetro associado aos níveis de refência "Asian/Não definido/Black" (variável Cor Pai) e "Nunca/Fumava" (variável Fuma).   
...
Analogamente, de acordo com o modelo ajustado, o fato da mãe fumar têm efeito no peso esperado do bebê equivalente (isoladamente e nas mesmas condições) a:
...
Como em geral estas variáveis são intrinsecas à mãe, ou seja, não estão sujeitas a alterações estipuladas, temos portanto, que os dados indicam uma concordância com o relato no Surgeon General's Report 1989, ou seja, o fato da mãe fumar parece ser uma variável mais determinante quanto ao peso do bebê do que altura, o peso, o numero de fetos, a renda anual da mãe, o histórico de resultados de gestações anteriores ou o sexo do bebê.
