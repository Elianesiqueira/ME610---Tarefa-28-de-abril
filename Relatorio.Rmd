...
Excluindo-se as variáveis indicadas como não significativas pelo ajuste anterior obteve-se o segundo modelo ajustado ao qual indicou que os níveis "Fumava" (referente a variável "Fuma"), "Não definido" (referente a variável "Cor Pai") e "Mex","Mixed" e "Não Definido" (referentes a variável "Cor Mãe") não são estatisticamente significantes ao nível de 10% de significância, ou seja, estes níveis não são estatisticamente diferentes dos seus respectivos níveis de referência "Nunca" (refente a variável "Fuma"), "Asian" (referente a variável "Cor do Pai") e "Asian" (referente a variável "Cor da mãe"). 
...
O terceiro modelo foi então ajustado com as mesmas variáveis que o segundo, porém com as alterações nos níveis indicadas. Deste modelo observou-se que os níveis "Black" (referente a variável "Cor Pai") e "white" (referente a variável "Cor Mãe") não são estatisticamente diferentes dos respectivos níveis de referência "Asian/Não definido" (referente a variável "Cor do Pai") e "Asian/Mex/Mixed/Não Definido" (referente a variável "Cor da Mãe"), portanto, como anteriormente, agrupou-se os grupos em "Asian/Não definido/Black" (referentes a cor do pai) e em "Asian/Mex/Mixed/Não Definido/white" (referentes a cor da mãe).
Com as devidas alterações nos níveis das variáveis citadas, e com as mesmas variáveis do terceiro modelo, foi-se ajustado o quarto modelo, onde observou-se que o nível "Black" referente a variável "Cor da Mãe" não é estatisticamente diferente do grupo de referência que engloba todos os outros níveis da variável "Cor da Mãe", portanto, a variável "Cor da Mãe", considerando o critério adotado, não é estatisticamente significante e deve ser retirada de um próximo modelo.
...
$Fuma$: é o nível "Fuma" da variável "Fuma".  
$Mex$: é o nível "Mex" da variável "Cor do Pai".  
$Mixed$: é o nível "Mixed" da variável "Cor do Pai".  
$White$: é o nível "White" da variável "Cor do Pai".  
$P$: é o valor observado da variável "Peso_mae_kg".  
$G$: é o valor observado da variável "Tempo_gestacao".  
$N$: é o valor observado da variável "Data_nasc".  
$NG$: é o valor observado da variável "Numero_gestacoes".  
$A$: é o valor observado da variável "Altura_mae_cm".  
$X = P,G,N,NG,A$.  
$\bar{X}$ é a média observada da variável X.  
$\sigma_X$ é o desvio padrão observado na variável X.
$Peso$: representa a variável resposta Peso.
...
Com base nos resultados obtidos neste estudo, observamos uma variável no modelo não tão intuitiva para explicar o peso do bebê, porém seus coeficientes indicam uma grande influência no peso do bebê, esta variável é a Cor/Raça do Pai, porém como esta não é de interesse para sanar o objetivo deste estudo, não serão levantadas discussões sobre a relação entre Cor/Raça do Pai e Peso do bebê. 
Segundo as estimativas do modelo, o fato da mãe da criança fumar ou não têm grande impacto negativo quanto ao peso do bebê, por exemplo, a diferença esperada no peso do bebê de uma mãe não fumante (Nunca Fumou ou Fumava) comparada com uma mãe fumante com as mesmas características é igual à **0.2390**, valor este equivalente a **1.21** vezes o valor observado para o coeficiente referente ao Tempo de gestação, de modo que é equivalente ao aumento em **1.21** vezes uma unidade de desvio padrão observado de Tempo de Gestação da mãe. Segundo a tabela06 em anexo, o desvio padrão observado nos dados para o Tempo de gestação é **15.166**, ou seja, o peso esperado para o filho de uma mãe que fuma equivale ao peso esperado para o filho de uma mãe com as mesmas características, porém que não fuma e que teve aproximadamente **18** dias a menos de gestação.
Analogamente, de acordo com o modelo ajustado, o fato da mãe fumar tem efeito no peso esperado do bebê equivalente (isoladamente e nas mesmas condições) a:
